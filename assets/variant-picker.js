...
export default class VariantPicker extends Component {
    ...
    connectedCallback() {
        ...
        // Initialize variant images from the data attribute
        try {
            this.#variantImages = JSON.parse(
                this.dataset.variantImages || "{}"
            );
        } catch (e) {
    ...
    variantChanged(event) {
        ...
        // First dispatch the regular variant selected event
        this.dispatchEvent(
            new VariantSelectedEvent({
                id: event.target.dataset.optionValueId ?? "",
            })
        );

        // Get the selected option and its images
        const selectedOption1 = this.querySelector(
            "select:first-of-type option[selected], fieldset:first-of-type input:checked"
        );

        if (selectedOption1 instanceof HTMLElement) {
            const optionValue = selectedOption1.getAttribute("value") || "";
            const galleryImages = this.#variantImages[optionValue] || [];

            // Dispatch custom event for gallery images
            this.dispatchEvent(
                new CustomEvent("option1-selected", {
                    detail: {
                        value: optionValue,
        ...
        this.fetchUpdatedSection(
            this.buildRequestUrl(event.target),
            loadsNewProduct
        );
    ...
    buildRequestUrl(
        ...
        if (this.selectedOptionsValues.length && !source) {
            params.push(
                `option_values=${this.selectedOptionsValues.join(",")}`
            );
        } else if (source === "product-card") {
            if (this.selectedOptionsValues.length) {
                params.push(
                    `option_values=${sourceSelectedOptionsValues.join(",")}`
                );
            } else {
                params.push(
                    `option_values=${selectedOption.dataset.optionValueId}`
                );
            }
        }
    ...
    fetchUpdatedSection(requestUrl, shouldMorphMain = false) {
        ...
        fetch(requestUrl, { signal: this.#abortController.signal })
            ...
            .then((responseText) => {
                ...
                html.querySelector("overflow-list[defer]")?.removeAttribute(
                    "defer"
                );
